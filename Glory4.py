...
class GloryAI:
    ...
    def update_program(self, update_url):
        ...
        try:
            ...
            if response.status_code == 200:
                temp_file = "temp_update.py"
                with open(temp_file, "wb") as file:
                    file.write(response.content)
                
                current_file = __file__
                with open(temp_file, "r") as new_code:
                    with open(current_file, "w") as current:
                        current.write(new_code.read())
                os.remove(temp_file)
                self.restart_program()
                return "Das Programm wurde erfolgreich aktualisiert und neu gestartet."
            else:
